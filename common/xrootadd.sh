...
for inputFilePath in $inputDSPath/*; do
    ...
    if [[ $inputFileName == *.root* && $inputFileName != *.GLOBAL.* && $inputFileName != *.tgz* ]]; then
	...
	if [[ $(source /a/data/xenia/users/common/xrootls.sh $relativePath/$datasetName $outputFile) == $outputFile ]]; then
	    #it has been added, continue
	    print $relativePath/$datasetName/$outputFile has already been added to xrootd
		
	else
	    print
	    print xrdcp $inputFilePath xroot://xenia.nevis.columbia.edu:1094//xrootd/$relativePath/$datasetName/$outputFile
	    xrdcp $inputFilePath xroot://xenia.nevis.columbia.edu:1094//xrootd/$relativePath/$datasetName/$outputFile
	    
	    if [[ $? = 0 ]]; then
		print adding record of $outputFile to /a/data/xenia/users/common/fileCatalog/$relativePath/$datasetName
		echo $outputFile >>! /a/data/xenia/users/common/fileCatalog/$relativePath/$datasetName
		((nfilesadded++))
	    fi
	fi
	((nfiles++))
...
#Write to dataset catalog:
    print writing entry for dataset to /a/data/xenia/users/common/datasetLog
